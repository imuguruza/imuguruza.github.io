...
The origins of the VGA standard are back from when displays used cathode ray
tubes for creating images. These cathode ray tube based displays shoots electrons all over the display and start over again. We normally
refer to this frequency as _refresh rate_ of a display (45 or 60 Hz for example). So, we can see that VGA was designed to drive
analog displays.
...
![](https://static-assets.imageservice.cloud/3324566/fi-adapter-wiring-diagram-xbox-get-free-image-about-wiring-diagram.jpg)
...
the display when an horizontal line ends up and the entire display display has been refreshed, starting the cycle again from the top.
...
These last signals were used to give time to the cathode ray tubes to place from the end to the beginning.
...
at the beginning was when the vertical sync signal needs to be created. This signals holds high until the `hsync` has gone through all the 480 rows until
...
ladder and a DAC to create the analog signals for the RGB color. Each color has three input pins, so you can create plenty of them.
...
Once I was sure I could make it work, I designed the Verilog `vga sync` module. The goal of this module is to create the sync signals. As the Alhambra board has a 12 MHz oscillator,
and the required pixel clock I require was of 25.175 MHz, I needed to use one of the PLL the FPGA contains. The good thing is that Yosys supports it and it is straight forward to
instantiate it. This PLL allows to create the desired pixel clock out of the oscillator. If you want to understand how PLL works, I recommend you reading [this wiki](https://github.com/mystorm-org/BlackIce-II/wiki/PLLs), it explain how the iCE40 PLL works and it parameters.

Yosys has a helpful tool to know the value of the parameters PLL needs to be instantiated, `icepll`. I used it to see the parameter values:
...
This module is wrapped in a [test module](https://github.com/imuguruza/alhambra_II_test/blob/master/vga/vga_image/vga_image.v). This module instantiates the `vga sync` module and uses its horizontal and vertical counters to create RGB values when the counters
are in the display area. The test pattern shown are four squares, putting each of the pins of RGB outs at '1'. At fourth square, all the outs are set to '1', creating a
white square.
...
with the test palette. The idea it was to store the code of each pixel in the RAM. But first, I needed to check the amount of RAM the board contains. Checking the datasheet,
...
In order to optimize the use of memory, it would make sense to try to fit RGB color somehow in one of the four RAM configurations, not wasting bits. Playing a bit with RAM size, RGB bits and image sizes, I decided to use 8 bits RGB color (one would be unused) and a 100x100 size image. That would make, 100x100x8 = 8000 bits = 7,8125 Kbits of RAM usage. This means
that all of the 80Kb of RAM would be used in the design. As the RGB color bits set are 8, I decided to use 512x8 RAM configuration, storing one pixel RGB value in each address. This
...
we need to draw. This module also instantiates the `ram` module, passing the data width and address with and the image we want to load, creating an appropriate RAM configuration and
loading the image in the RAM.

Finally, using the aforementioned counters, I have added the logic to create in the middle of the display the image. The rest of the display will be fill out with a color. This could
...
Finally I saved that color string. In order to obtain a list of one pixel colors, I used first `tr -s , '\n' < string.file` command. This command searches for colons and convert them
into line breaks. This allowed me to have each pixel color in different lines. Finally, using a test editor, I erased all the `0x` that appeared in each line ( I couldn't do it using
`tr`, it was erasing all the 0s, deleting all of them in many lines). Using this procedure, I have created Futurama's Bender and Fry image lists.
...
It is pretty annoying to need to change the image file at the top module and re-synthesize each time you want to display a new image.
So, I would like to add a serial communication with a buffer. This would allow to send a new image file and it could be possible to be stored in the RAM of the FPGA. Probably, this would need an additional
RAM block, attached to the serial interface to store the image. Once the image is sent, this new image would be loaded in the VGA module RAM, changing the displayed pattern. This implies the need of two separate RAM blocks, diminishing the size of the displayed image, as I have used almost the 100% of the RAM the board contains.

This also implies having two clock domains, one for the serial communication and one for creating VGA signals. Let's see if I can put some time to develop this functionality.
...
+ [Digikey page](https://www.digikey.com/eewiki/pages/viewpage.action?pageId=15925278)
